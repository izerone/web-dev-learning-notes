...
描述算法的性能或复杂度的形式化方法是**大 O 符号** ( Big O Notation ) ，表示随着输入变化而发生的性能变变化。 如 $O(n^2)$  。 $O(n^2)$ 并不是最高效的算法，算法高效与否完全取决于算法的具体目标。

大 $O$ 符号通常适用于较大的 $n$ 值 。当 $n$ 很小时，算法的**运行时间**和**空间消耗**都可以忽略不计。 对于编写的任何算法，都应该考虑其 大 $O$ 符号复杂度，即使 $n$ 的值很小。
